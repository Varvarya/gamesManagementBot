...
export class ClubManagementHandler {
    ...
    async handleCallback(ctx: Context): Promise<boolean> {
        ...
        if (callback === 'onboarding:join') { await ctx.reply('Оберіть клуб зі списку доступних клубів.'); return true; }
    ...
    private async showSuperAdminEntry(ctx: Context): Promise<true> { await ctx.reply('🌐 Суперадміністратор', Markup.inlineKeyboard([[Markup.button.callback('🌐 Панель суперадміністратора', 'superadmin:menu')], [Markup.button.callback('🏸 Перейти до клубу', 'mode:club_select')]])); return true; }
    ...
    private async showClub(ctx: Context, id: string): Promise<true> { const club = await this.clubs.findById(id); if (!club) { await ctx.reply('Клуб не знайдено.'); return true; } await ctx.reply(`🏸 ${club.name}\n\n👥 Адміністратори: ${club.admins.length}`, Markup.inlineKeyboard([[Markup.button.callback('👥 Адміністратори', `club:admins:${id}`)], [Markup.button.callback('⚙️ Налаштування', `club:settings:${id}`)], [Markup.button.callback('🗑 Видалити клуб', `club:delete:${id}`)], [Markup.button.callback('⬅️ Назад', 'clubs:list')]])); return true; }

...
test('ready preview exposes Import only after all blockers are resolved', () => {
    const blocked = { id: 'Ab31cd', state: 'reviewing', canCommit: false, possibleDuplicateCount: 0, reviewCount: 0, blockedCount: 4, importCandidates: [{ importCandidateId: 'candidate-ready-check-1', token: 'C1', candidate: {} }], reviewCandidates: [], decisions: {}, plan: { newCount: 1, updateCount: 0, conflicts: [{ type: 'ambiguous_exact_match', rows: [2], candidatePlayerIds: ['p'] }] } } as unknown as TelegramPlayerImportSession;
...
test('production-shaped reviewing state derives Review from the next candidate, not conflict metadata flags', () => {
    const candidate = { importCandidateId: 'candidate-production-1', token: 'candidate1', candidate: { telegramUserId: 101, telegramDisplayName: 'Олександр', suggestedDisplayName: 'Олександр', aliases: [], isContact: false, needsReview: false } };
...
test('normalized import UI state matrix never hides a forward action for resolvable non-terminal states', () => {
    ...
    const candidate = { importCandidateId: 'candidate-matrix-soft-1', token: 'soft1', candidate: { telegramUserId: 1, telegramDisplayName: 'Name', suggestedDisplayName: 'Name', aliases: [], isContact: false, needsReview: false }, candidatePlayerIds: [] };
    const cases: Array<[string, TelegramPlayerImportSession, string[]]> = [
        ...
        ['mixed', { ...base, state: 'reviewing', blockedCount: 2, reviewCount: 1, reviewCandidates: [{ ...candidate, type: 'NEEDS_REVIEW' }], importCandidates: [{ ...candidate, importCandidateId: 'candidate-matrix-hard-1', token: 'hard1' }], plan: { ...base.plan, blockedCount: 1, canCommit: false, conflicts: [{ type: 'ambiguous_exact_match', rows: [2], message: 'ambiguous' }] } }, ['review', 'skip_problematic', 'cancel']],

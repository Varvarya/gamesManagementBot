...
test('opening upcoming repeatedly leaves automatic publication job intact and it still fires once', async (t) => {
    ...
    jobs.set('club:club-a:training:one-off', async () => undefined);
    await automatic.restore([template]);
    const jobIds = [...jobs.keys()];
    assert.equal(jobs.has('club:club-a:training:one-off'), true, 'template restore must not cancel concrete-training jobs');
    ...
    await jobs.get(jobIds.find((id) => id.includes(':template:'))!)!();
    assert.equal(publications, 1);
});

test('startup recovers the exact overdue 20.08.2026 18:00 recurring occurrence in Kyiv', async (t) => {
    const { root, repositories } = await fixture();
    t.after(() => rm(root, { recursive: true, force: true }));
    await repositories.chats.create({ id: -100, name: 'WAVE Badminton Club', enabled: true });
    const template: TrainingTemplate = {
        id: 'automatic', clubId: 'club-a', chatId: -100, title: 'Training', placesLimit: 12, minPlayers: 4,
        publishDaysBefore: 1, publishTime: '18:10', enabled: true,
        slots: [{ id: 'thursday', dayOfWeek: 4, startTime: '18:00', endTime: '20:00', enabled: true }], createdAt: '', updatedAt: '',
    };
    await repositories.templates.save(template);
    const services = new ServicesContext(repositories); let publications = 0; let publishedDate = '';
    const scheduler = { cancelByPrefix: () => undefined, cancelTemplate: () => undefined, rescheduleTemplate: () => undefined, getJobsSnapshot: () => [], getScheduledTemplateIds: () => [] } as unknown as SchedulerService;
    const publisher = { publishTemplateSlot: async (input: { date: string }) => { publications++; publishedDate = input.date; return { id: 'training', status: 'open', messageId: 42 }; } } as unknown as TrainingPublisherService;
    const automatic = new TemplateSchedulerService(services.templates, scheduler, publisher, services.chats, repositories.settings, () => new Date('2026-08-20T08:00:00.000Z'));
    await automatic.restore([template], { reconcileMissed: true });
    assert.equal(publications, 1);

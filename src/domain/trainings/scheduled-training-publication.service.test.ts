...
function fixture(now = '2026-08-19T12:00:00.000Z') {
    let value = base(); let publications = 0; let fail = false;
    ...
    const publisher = { publishExistingDraft: async () => { publications++; if (fail) throw new Error('telegram'); value = { ...value, status: 'open', messageId: 42, publishedAt: 'now', scheduledPublicationAt: undefined }; return value; } } as unknown as TrainingPublisherService;
    ...
    return { service, jobs, get value() { return value; }, set value(next: Training) { value = next; }, get publications() { return publications; }, fail() { fail = true; } };
}
...
test('future publication is scheduled in Europe/Kyiv and firing reloads and publishes it', async () => {
    ...
    assert.deepEqual(job.input, { id: 'club:club:training:training', date: '2026-08-19', time: '18:10', timezone: 'Europe/Kyiv' });
    await job.run();
